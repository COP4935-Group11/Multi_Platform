...
CucumberKW.runFeatureFile('Include/features/Content-Content-Authoring/uploadDocumentsToContentModule.feature')
...
def teardown()
{
   ...
   if (!support.isInTestSuite())
   {
      // Login as orgadmin, remove documents, and logout
      support.login(GlobalVariable.orgadmin_username, GlobalVariable.orgadmin_password, GlobalVariable.orgadmin_key)
      SharedTestData data = SharedTestData.getInstance()
      data.deleteContentModuleDocuments(use_content_module_named)
      support.logout()
      
      // Then, login as author, remove content module itself, and logout
      support.login(GlobalVariable.author_username, GlobalVariable.author_password, GlobalVariable.author_key)
