...
CucumberKW.runFeatureFile('Include/features/Event-Outlook/verifySurvey.feature')
...
def setup()
{
   ...
   if (!support.isInTestSuite())
   {
      ...
      // Make a lab event
      data.createLabEvent(use_lab_event_named, use_training_package_named, use_survey_named)

      // Then, end it and close it so users can take survey
      data.endLabEvent(use_lab_event_named)
...
def teardown()
{
   ...
   if (!support.isInTestSuite())
   {
      // Login as manager and remove lab event
      support.login(GlobalVariable.manager_username, GlobalVariable.manager_password, GlobalVariable.manager_key)
      SharedTestData data = SharedTestData.getInstance()
      data.deleteLabEvent(use_lab_event_named, true)
      support.logout()
