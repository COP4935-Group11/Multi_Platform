...
CucumberKW.runFeatureFile('Include/features/Content-Content-Authoring/createTrainingPackage.feature')
...
def setup()
{
   ...
   data.addTestParam("trainingPackageName", "PCTE Creation Test " + test_code)
   data.addTestParam("contentModuleToUse", use_content_module_named)

   // Create support test data if running standalone (outside a test suite that would make it)
